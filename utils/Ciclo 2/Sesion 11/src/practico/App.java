...
public class App {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < c_prendas; i++) {
            ...
            v_prendas[i] = new Prendas(cod_prenda, des_prenda, p_compra, p_venta, c_bodega, c_minima, c_maxima,
                    t_prenda, planchado);
        }
        ...
        for (int i = 0; i < c_calzado; i++) {
            System.out.println("Códig Calzado: ");
            ...
            v_calzado[i] = new Calzado(cod_calzado, des_calzado, p_compra_calzado, p_venta_calzado, c_bodega_calzado,
                    c_minima_calzado, c_maxima_calzado, t_calzado);
        }
        int opt = 0;
        do {
            ...
            switch (opt) {
                ...
                case (5): // Modificar cantidad mínima requerida en bodega:
                    lector.nextLine();
                    System.out.println("Tipo de producto  [P: prenda | C: calzado]: ");
                    ...
                    if (tipo.equals("P")) {
                        ...
                        while (i < c_prendas && sw == 0) {
                            if (v_prendas[i].codigo == cod) {
                                System.out.println("ELEMENTO ENCONTRADO");
                                System.out.println("Nueva cantidad min. requerida: ");
                                int nueva_cant_req = lector.nextInt();
                                if (v_prendas[i].cantidad_minima < 0) {
                                    v_prendas[i].cantidad_minima = 1;
                                } else {
                                    v_prendas[i].cantidad_minima = nueva_cant_req;
                                }
                                System.out.println("Nueva cantidad min. req_" + v_prendas[i].cantidad_minima);
                                sw = 1;
                            }
                            i++;
                        }
                        ...
                        while (i < c_calzado && sw == 0) {
                            ...
                            System.out.println("Nueva cantidad min. requerida: ");
                            int nueva_cant_req = lector.nextInt();
                            if (v_calzado[i].cantidad_minima < 0) {
                                v_calzado[i].cantidad_minima = 1;
                            } else {
                                v_calzado[i].cantidad_minima = nueva_cant_req;
                            }
                            System.out.println("Nueva cantidad min. req_" + v_calzado[i].cantidad_minima);
                            sw = 1;
                            i++;
                        }
                        if (sw == 0) {
                    ...
                    if (tipo.equals("P")) {
                        ...
                        while (i < c_prendas && sw == 0) {
                            if (v_prendas[i].codigo == cod) {
                                ...
                                int unidades_venta = lector.nextInt();
                                if (v_prendas[i].cantidad_bodega >= unidades_venta) {
                                    System.out.println("Factura sin dcto: " + v_prendas[i].calcularPagar(unidades_venta, 0));
                                    System.out.println("Factura con dcto: " + v_prendas[i].calcularPagar(unidades_venta, 1));
                                    v_prendas[i].cantidad_bodega -= unidades_venta;
                                    System.out.println(v_prendas[i]);
                                } else {
                        ...
                        while (i < c_calzado && sw == 0) {
                            if (v_calzado[i].codigo == cod) {
                                ...
                                System.out.println("Unidades a vender: ");
                                int unidades_venta = lector.nextInt();
                                if (v_calzado[i].cantidad_bodega >= unidades_venta) {
                                    System.out.println("Factura sin dcto: " + v_calzado[i].calcularPagar(unidades_venta, 0));
                                    System.out.println("Factura con dcto: " + v_calzado[i].calcularPagar(unidades_venta, 1));
                                    v_calzado[i].cantidad_bodega -= unidades_venta;
                                    System.out.println(v_calzado[i]);
                                } else {
                                    System.out.println("No se puede vender");
                                }
                ...
                default:
                    System.out.println("Que ejercicio largo");
                    break;
            }
